...
describe("validarCandidato — cada regla de negocio produce el código de conflicto esperado", () => {
  ...
  it("permite sábado para un grupo de 7.º semestre", () => {
    const ctx = baseContexto({
      ...
      docentes: [{ id: "d-1", tipo_contrato: "por_horas", hora_entrada: null, hora_salida: null, max_horas_semana: 20, sede_principal_id: "s-1", disponibilidad: [1, 2, 3, 4, 5, 6].map((dia_semana) => ({ dia_semana: dia_semana as 1 | 2 | 3 | 4 | 5 | 6, hora_inicio: "08:00", hora_fin: "17:00", es_tiempo_oficina: false })) }],
    });
    const resultado = validarCandidato(slot({ dia: 6 }), ctx, []);
  ...
  it("DOCENTE_NO_DISPONIBLE cuando el docente no tiene disponibilidad ese día", () => {
    const ctx = baseContexto({ docentes: [{ id: "d-1", tipo_contrato: "por_horas", hora_entrada: null, hora_salida: null, max_horas_semana: 20, sede_principal_id: "s-1", disponibilidad: [{ dia_semana: 1, hora_inicio: "08:00", hora_fin: "17:00", es_tiempo_oficina: false }] }] });
    const resultado = validarCandidato(slot({ dia: 2 }), ctx, []);
    expect(resultado).toEqual({ valida: false, conflicto: expect.objectContaining({ codigo: "DOCENTE_NO_DISPONIBLE" }) });
  });

  it("DOCENTE_NO_DISPONIBLE cuando la franja cae en tiempo de oficina", () => {
    const ctx = baseContexto({
      docentes: [{
        id: "d-1", tipo_contrato: "por_horas", hora_entrada: null, hora_salida: null, max_horas_semana: 20, sede_principal_id: "s-1",
        disponibilidad: [
  ...
  it("EXCEDE_MAX_HORAS cuando la sesión supera la carga semanal del docente", () => {
    const ctx = baseContexto({ docentes: [{ id: "d-1", tipo_contrato: "por_horas", hora_entrada: null, hora_salida: null, max_horas_semana: 0.5, sede_principal_id: "s-1", disponibilidad: [1, 2, 3, 4, 5].map((dia_semana) => ({ dia_semana: dia_semana as 1 | 2 | 3 | 4 | 5, hora_inicio: "08:00", hora_fin: "17:00", es_tiempo_oficina: false })) }] });
    const resultado = validarCandidato(slot(), ctx, []);
    expect(resultado).toEqual({ valida: false, conflicto: expect.objectContaining({ codigo: "EXCEDE_MAX_HORAS" }) });
  });
