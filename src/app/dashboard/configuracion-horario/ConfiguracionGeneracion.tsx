...
/** Preparación reutilizable que acompaña al flujo de generación de horario. */
export async function ConfiguracionGeneracion() {
  ...
  return <div className="space-y-6">
    <div>
      <span className="font-mono text-xs uppercase tracking-wider text-[#1D3FD9]">Configuración de generación</span>
      <h2 className="mt-1 text-xl font-semibold tracking-tight text-[#0E1116]">Prepara los datos antes de generar</h2>
      <p className="mt-1 text-sm text-gray-600">Asigna docentes y ajusta la disponibilidad de las aulas sin salir de este flujo.</p>
    </div>
    ...
    <section className="rounded-xl border bg-white p-5 space-y-5">
      ...
      <form action={guardarDisponibilidadEspacio} className="grid gap-3 md:grid-cols-5 items-end">
        ...
        <label className="grid gap-1 text-sm">Día *<select name="dia_semana" className="min-w-0 rounded-lg border p-2.5">{DIAS.map((dia, index) => <option key={dia} value={index + 1}>{dia}</option>)}</select></label>
        ...
        <label className="grid gap-1 text-sm">Hasta *<input name="hora_fin" type="time" min="08:00" max="20:00" step="1800" defaultValue="17:00" required className="rounded-lg border p-2.5" /></label>
        <div><label className="mb-2 flex gap-2 text-sm"><input name="disponible" type="checkbox" defaultChecked /> Disponible</label><PendingSubmitButton pendingLabel="Guardando…" className="inline-flex items-center gap-2 rounded-lg bg-[#1D3FD9] px-4 py-2.5 text-sm font-semibold text-white">Guardar franja</PendingSubmitButton></div>
      </form>
