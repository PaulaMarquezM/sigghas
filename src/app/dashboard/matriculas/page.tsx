...
export default async function MatriculasPage() {
  ...
  const listadoPeriodos = (periodos.data ?? []) as unknown as Array<{ id: string; activo: boolean }>;
  const activo = listadoPeriodos.find((periodo) => periodo.activo);
  const respuestaHorarios = activo ? await db.from("horarios").select("id").eq("periodo_id", activo.id).eq("estado", "publicado").order("generado_en", { ascending: false }) : { data: [] };
  const horarios = (respuestaHorarios.data ?? []) as unknown as Array<{ id: string }>;
